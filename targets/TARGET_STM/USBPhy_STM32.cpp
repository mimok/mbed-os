...
#define PMA_EP0_IN_ADDR     (PMA_EP0_OUT_ADDR + MAX_PACKET_SIZE_EP0)
    MBED_ASSERT(HAL_PCDEx_PMAConfig(&hpcd, LOG_OUT_TO_EP(0), PCD_SNG_BUF, PMA_EP0_OUT_ADDR) == HAL_OK);
    MBED_ASSERT(HAL_PCDEx_PMAConfig(&hpcd, LOG_IN_TO_EP(0),  PCD_SNG_BUF, PMA_EP0_IN_ADDR) == HAL_OK);
    // EP1
...
#define PMA_EP1_CMD_ADDR    (PMA_EP1_IN_ADDR + MAX_PACKET_SIZE_NON_ISO)
    MBED_ASSERT(HAL_PCDEx_PMAConfig(&hpcd, LOG_OUT_TO_EP(1), PCD_SNG_BUF, PMA_EP1_OUT_ADDR) == HAL_OK);
    MBED_ASSERT(HAL_PCDEx_PMAConfig(&hpcd, LOG_IN_TO_EP(1),  PCD_SNG_BUF, PMA_EP1_CMD_ADDR) == HAL_OK);
    // EP2
...
#define PMA_EP2_CMD_ADDR    (PMA_EP2_IN_ADDR + MAX_PACKET_SIZE_NON_ISO)
    MBED_ASSERT(HAL_PCDEx_PMAConfig(&hpcd, LOG_OUT_TO_EP(2), PCD_DBL_BUF, PMA_EP2_OUT_ADDR) == HAL_OK);
    MBED_ASSERT(HAL_PCDEx_PMAConfig(&hpcd, LOG_IN_TO_EP(2),  PCD_SNG_BUF, PMA_EP2_CMD_ADDR) == HAL_OK);
    // EP3
...
#define PMA_EP3_CMD_ADDR    (PMA_EP3_IN_ADDR + MAX_PACKET_SIZE_ISO)
    MBED_ASSERT(HAL_PCDEx_PMAConfig(&hpcd, LOG_OUT_TO_EP(3), PCD_SNG_BUF, PMA_EP3_OUT_ADDR) == HAL_OK);
    MBED_ASSERT(HAL_PCDEx_PMAConfig(&hpcd, LOG_IN_TO_EP(3),  PCD_SNG_BUF, PMA_EP3_CMD_ADDR) == HAL_OK);
    ...
    fifo_size =  MAX_PACKET_SIZE_EP0 + 4;
    HAL_PCDEx_SetTxFiFo(&hpcd, 0, fifo_size / 4);
    total_bytes += fifo_size;

    fifo_size =  MAX_PACKET_SIZE_NON_ISO + 4;
    HAL_PCDEx_SetTxFiFo(&hpcd, 1, fifo_size / 4);
    total_bytes += fifo_size;

    fifo_size =  MAX_PACKET_SIZE_NON_ISO + 4;
    HAL_PCDEx_SetTxFiFo(&hpcd, 2, fifo_size / 4);
    total_bytes += fifo_size;

    fifo_size =  MAX_PACKET_SIZE_ISO + 4;
    HAL_PCDEx_SetTxFiFo(&hpcd, 3, fifo_size / 4);
    total_bytes += fifo_size;
...
void USBPhyHw::connect()
{
    ...
    DigitalOut usb_disc_pin(PG_6, 1) ;
    wait(0.01);
    usb_disc_pin = 0;
#endif
#if (MBED_CONF_TARGET_USB_SPEED == USE_USB_NO_OTG)
    ...
    MBED_ASSERT(HAL_PCD_DevConnect(&hpcd) == HAL_OK);
    HAL_Delay(4);
#else
